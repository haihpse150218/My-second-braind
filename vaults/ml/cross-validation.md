...
title: Kiểm định chéo (Cross-Validation) & Chia Train/Test
vault: ml
type: concept
branch: F
order: 5
...
prev: [overfitting, huan-luyen-vs-suy-luan]
sources: [L2_Intro_ML_DL_GenAI.pdf]
created: 2026-06-14
---
...
**Dùng trong:** [[../../projects/dsp-urbansound|📦 dsp-urbansound]]
**Tags:** #ml #danh-gia #cross-validation #lop-lam
...
## 🔀 Các biến thể K-Fold *(L6 mở rộng)*
| Biến thể | Làm gì | Khi nào dùng |
|----------|--------|--------------|
| **K-Fold** | chia K phần ngẫu nhiên | data **cân bằng** |
| **Stratified K-Fold** | giữ **tỉ lệ lớp** trong mỗi fold | **mất cân bằng** → [[class-imbalance]]; **mặc định của sklearn cho classification** |
| **Repeated (Stratified) K-Fold** | lặp K-Fold nhiều lần với seed khác → giảm variance của ước lượng | cần con số ổn định hơn (tốn thời gian hơn) |

```python
from sklearn.model_selection import StratifiedKFold, RepeatedStratifiedKFold, cross_val_score

# Stratified K-Fold — giữ tỉ lệ lớp
skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
scores = cross_val_score(model, X, y, cv=skf, scoring='accuracy')

# Repeated Stratified — 3 lần × 5-fold = 15 lần đánh giá
rskf = RepeatedStratifiedKFold(n_splits=5, n_repeats=3, random_state=42)
scores = cross_val_score(model, X, y, cv=rskf)
print(f'{scores.mean():.3f} ± {scores.std():.3f}')
```
> 💡 `cross_val_score(model, X, y, cv=5)` cho bài **classification** đã **tự dùng Stratified K-Fold** — truyền số nguyên là đủ trong đa số trường hợp.
...
```
- Công cụ: **GridSearchCV** (thử mọi tổ hợp) / **RandomizedSearchCV** (thử ngẫu nhiên) trong scikit-learn → chi tiết quy trình + đường cong tại [[hyperparameter-tuning]].
