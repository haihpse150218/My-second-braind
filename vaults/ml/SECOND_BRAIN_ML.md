...
**Nguồn đã học:** ✅ `L1_Math_Overview.pdf` (5 mảng toán — [[note]]) · ✅ `L2_Intro_ML_DL_GenAI.pdf` (6 mục) · ✅ `D:\MSA-FPT\Machine learning\slide\L3_LinearReg.pdf` (Hồi quy tuyến tính → [[linear-regression]] · [[metric-hoi-quy]]) · ✅ `D:\MSA-FPT\Machine learning\slide\L4_LogisticsReg.pdf` (Hồi quy Logistic & phân loại → [[logistic-regression]] · [[softmax]] · [[class-imbalance]] · [[danh-gia-mo-hinh]]) · ✅ `D:\MSA-FPT\Machine learning\slide\L5_Regularization_FeatureSelection.pdf` (Tiền xử lý · Regularization · Chọn đặc trưng → [[regularization]] · [[feature-selection]] · [[bias-variance]]) · ✅ `D:\MSA-FPT\Machine learning\slide\L6_DecisionTree.pdf` (Cây quyết định: tiêu chí chia Gini/Entropy · overfitting · CV · tinh chỉnh siêu tham số → [[decision-tree]] · [[hyperparameter-tuning]] · [[cross-validation]]) · ✅ `D:\MSA-FPT\Machine learning\slide\L7_Ensemble.pdf` (Ensemble: Bootstrap/Bagging · Random Forest · AdaBoost · Gradient Boosting/XGBoost · bias-variance → [[ensemble-learning]] · [[random-forest]] · [[adaboost]] · [[xgboost]]) · ✅ `D:\MSA-FPT\Machine learning\slide\L8_NeuralNetwork.pdf` (Neural Network / MLP: neuron=LR · activation · forward pass · loss · gradient descent · backpropagation · vanishing gradient → [[neural-network]])
...
1. [[decision-tree]] — 🌳 *(L6)* cây quyết định: if/else chia dữ liệu, Gini/Entropy + Info Gain (white-box, dễ overfit)
   - [[hyperparameter-tuning]] — *(L6)* GridSearch vs RandomSearch · learning vs validation curve · quy trình tune `max_depth`
...
4. [[ensemble-learning]] — 🎯 *(L7)* HUB họ ensemble: Bagging (∥→↓variance) vs Boosting (seq→↓bias) vs Stacking; wisdom of crowd
   - [[random-forest]] — 🌲 *(L7)* Bagging + feature randomness (m=√p → ρ thấp → ↓variance); OOB miễn phí; go-to baseline tabular
   - [[adaboost]] — 🥊 *(L7)* Boosting đầu tiên: reweight mẫu SAI, gộp stump theo trọng số α; nhạy noise
5. [[xgboost]] — 🚀 *(L7)* Gradient Boosting (fit residual = −gradient) + regularization + Newton bậc 2 → chuẩn vàng tabular
...
- [[ensemble-learning]] — 🎯 *(L7)* hub: Bagging vs Boosting vs Stacking (wisdom of crowd)
- [[random-forest]] — nhiều cây vote (bagging + feature randomness), OOB miễn phí, kháng nhiễu
- [[adaboost]] — *(L7)* boosting reweight mẫu sai (stump + α)
- [[xgboost]] — Gradient Boosting + regularization + Newton bậc 2, mạnh nhất cho dữ liệu bảng
...
- [[cross-validation]] — 🟢 chia Train/Test + K-Fold + Stratified/Repeated (đo đáng tin)
- [[hyperparameter-tuning]] — 🟢 *(L6)* tinh chỉnh siêu tham số: Grid/Random Search + learning/validation curve ⭐
...
- [[neural-network]] — 🧠 *(L8)* MLP & backpropagation: neuron=LR · activation · forward/backward pass · vanishing gradient ⭐
- [[pytorch-vs-tensorflow]] — 🔧 chọn framework DL (PyTorch vs TensorFlow + high/low-level API)
- *(chưa viết)* `[[dl/transfer-learning|transfer-learning]]` · `[[fine-tuning]]`

### 4. Công cụ & Thư viện
*(NumPy, Pandas, scikit-learn, PyTorch, TensorFlow...)*
- [[pytorch-vs-tensorflow]] — 🔧 chọn framework DL: PyTorch (nghiên cứu + HuggingFace) vs TensorFlow (triển khai + Keras); high-level vs low-level API
...
- 📑 Slide môn học (TS. Cao Tiến Dũng) — thư mục `D:\MSA-FPT\Machine learning\slide\`: `L1_Math_Overview.pdf` ✅ · `L2_Intro_ML_DL_GenAI.pdf` ✅ · `L3_LinearReg.pdf` ✅ · `L4_LogisticsReg.pdf` ✅ · `L5_Regularization_FeatureSelection.pdf` ✅ · `L6_DecisionTree.pdf` ✅ · `L7_Ensemble.pdf` ✅ · `L8_NeuralNetwork.pdf` ✅ · `DeCuong_HocMay_Summer2026.pdf`
  - 🔗 `D:\MSA-FPT\Machine learning\slide\ref-tree.md` — link tham khảo cách tính tay CART & ID3 (cho [[decision-tree]]).
- 🖼️ Hình tự vẽ: `eigenvector-pca.png` (Eigenvector & PCA) · `chon-mo-hinh.png` (sơ đồ chọn mô hình) · `bias-variance-target.png` (4 ô bia bắn Bias-Variance — [[bias-variance]])
