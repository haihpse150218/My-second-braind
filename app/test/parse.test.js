...
test('tiêu đề/tóm tắt chứa "Trạng thái" hoặc "Lộ trình" không làm lệch metadata', () => {
  const n = parseNote('trang-thai-h0', `# Trạng thái khởi động h0

> Tóm tắt 1 câu: Lộ trình nào cũng cần một trạng thái ban đầu.
...
`);
  ...
  assert.equal(n.order, 6);
  assert.deepEqual(n.prev, ['padding-masking']);
  assert.deepEqual(n.next, ['bptt']);
});
