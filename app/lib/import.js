...
// Fallback khi vault không khai báo "branches" (giữ hành vi cũ của vault ML).
export const DEFAULT_BRANCHES = `A: Giải tích → Tối ưu hóa
...
export function buildSystemPrompt(existingNotes, branches = DEFAULT_BRANCHES) {
...
export function buildRequest({ dataBase64, existingNotes, branches }) {
  return {
    model: MODEL,
    max_tokens: 16000,
    system: buildSystemPrompt(existingNotes, branches),
...
export async function importSlide({ dataBase64, existingNotes, apiKey, branches }) {
  const client = new Anthropic({ apiKey });
  const res = await client.messages.create(buildRequest({ dataBase64, existingNotes, branches }));
