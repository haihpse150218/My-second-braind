...
// bảng màu nhận diện nhánh (chỉ cho khung làn + nhãn, không phải status).
// Gán theo THỨ TỰ nhánh nên số nhánh bao nhiêu cũng chạy được.
const BRANCH_PALETTE = ['#7aa2f7', '#bb9af7', '#f7768e', '#9ece6a', '#e0af68', '#7dcfff', '#ff9e64'];
const VAULT_KEY = 'sb.vault';

const state = { notes: [], bySlug: new Map(), current: null, raw: '', graphMode: 'path',
  filter: 'all', query: '', sequence: [],
  vaults: [], vault: null, branchNames: {}, branchColor: {} };
let cy = null;

const $ = (sel) => document.querySelector(sel);

// Mọi lời gọi API đều phải kèm vault đang chọn.
// Ở chế độ tĩnh (mở bằng file:// hoặc GitHub Pages, không có server), SBStatic
// chặn tại đây và trả dữ liệu từ index.json — phần còn lại của app không đổi.
function api(path, opts) {
  if (window.SBStatic && window.SBStatic.on) {
    return window.SBStatic.handle(path, opts, state.vault);
  }
  const sep = path.includes('?') ? '&' : '?';
  return fetch(state.vault ? `${path}${sep}vault=${encodeURIComponent(state.vault)}` : path, opts);
}

// Nạp cấu hình nhánh của vault đang chọn (tên hiển thị + màu theo thứ tự).
function applyVaultConfig() {
  const v = state.vaults.find((x) => x.id === state.vault) || state.vaults[0];
  const branches = (v && v.branches) || {};
  state.branchNames = {};
  state.branchColor = {};
  Object.keys(branches).forEach((key, i) => {
    state.branchNames[key] = `Nhánh ${key} · ${branches[key]}`;
    state.branchColor[key] = BRANCH_PALETTE[i % BRANCH_PALETTE.length];
  });
  const name = (v && v.label) || 'Second Brain';
  document.title = `🧠 Second Brain — ${name}`;
  const brand = $('.brand');
  if (brand) brand.textContent = `🧠 Second Brain — ${name}`;
}
...
function computeSequence() {
  ...
  for (const b of Object.keys(state.branchNames).filter((x) => groups[x])) {
...
function renderSidebar() {
  ...
  const order = Object.keys(state.branchNames).filter((b) => groups[b]);
  let shown = 0;
  for (const b of order) {
    ...
    g.innerHTML = `<h3>${state.branchNames[b]} — ${done}/${all.length}</h3>
    ...
    items.forEach((n, row) => {
      ...
      elements.push({
        data: {
          ...
          branch: state.branchColor[n.branch] || '#565f89',
        },
        classes: 'note',
      });
    });
...
async function openNote(slug) {
  const res = await api(`/api/note/${slug}`);
  if (!res.ok) return;
...
$('#btn-save').onclick = async () => {
  ...
  const res = await api(`/api/note/${state.current}`, {
    method: 'PUT',
...
$('#status-select').onchange = async (e) => {
  ...
  const res = await api(`/api/note/${state.current}/status`, {
    method: 'PATCH',
    headers: { 'content-type': 'application/json' },
...
$('#import-file').onchange = async () => {
  ...
  try {
    ...
    const res = await api('/api/import-slide', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
...
$('#import-save').onclick = async () => {
  ...
  try {
    const res = await api('/api/import-save', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
...
function renderVaultSwitcher() {
  const box = $('#vault-switch');
  if (!box || state.vaults.length < 2) return;
  box.innerHTML = '';
  for (const v of state.vaults) {
    const b = document.createElement('button');
    b.textContent = v.label;
    b.className = v.id === state.vault ? 'active' : '';
    b.onclick = () => switchVault(v.id);
    box.appendChild(b);
  }
  box.hidden = false;
}
...
async function switchVault(id) {
  if (id === state.vault) return;
  state.vault = id;
  localStorage.setItem(VAULT_KEY, id);
  // đổi vault → note đang mở không còn ý nghĩa
  state.current = null;
  state.raw = '';
  $('#note-title').textContent = 'Chọn một note';
  $('#viewer').innerHTML =
    '<div class="empty-state"><div class="es-icon">🧠</div><p>Chọn một note ở cột trái để bắt đầu học.</p></div>';
  $('#note-nav').hidden = true;
  applyVaultConfig();
  renderVaultSwitcher();
  await loadNotes();
...
// Cho views.js điều khiển: mở note ở vault bất kỳ, đổi vault.
window.SBApp = {
  state,
  async gotoVault(id) {
    if (window.SBViews) window.SBViews.setTab('learn');
    await switchVault(id);
  },
  async gotoNote(vault, slug) {
    if (window.SBViews) window.SBViews.setTab('learn');
    if (vault && vault !== state.vault) await switchVault(vault);
    await openNote(slug);
  },
};
...
async function init() {
  let mode = 'server';
  try {
    // SBStatic tự dò: có server thì dùng API, không thì rơi về index.json
    const boot = window.SBStatic ? await window.SBStatic.boot() : null;
    const vaults = boot ? boot.vaults : (await (await fetch('/api/vaults')).json()).vaults || [];
    mode = boot ? boot.mode : 'server';
    state.vaults = vaults;
    const saved = localStorage.getItem(VAULT_KEY);
    state.vault = vaults.some((v) => v.id === saved) ? saved : (vaults[0] && vaults[0].id) || null;
    applyVaultConfig();
    renderVaultSwitcher();
  } catch {
    // server kiểu cũ (1 vault, không có /api/vaults) → vẫn chạy được
  }
  if (mode === 'static') {
    // Không có server → ẩn mọi nút ghi để không bấm vào rồi báo lỗi khó hiểu.
    for (const n of document.querySelectorAll('.rw-only, #btn-edit, #status-select')) n.hidden = true;
    toast('Chế độ chỉ đọc (index.json tĩnh)');
  }
  await loadNotes();
}
